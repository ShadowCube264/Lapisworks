...
public class EnchantFallDmgRes implements SpellAction {
    public int getArgc() {
        return 1;
    }
    ...
    public SpellAction.Result execute(List<? extends Iota> args, CastingEnvironment ctx) {
        ...
        if (((LapisworksInterface)entity).checkFallDmgRes() == 2) {
        ...
        if (offHandItems.isEmpty()) {
            ...
            MishapThrowerJava.throwMishap(MishapBadOffhandItem.of(offHandItems, "amel"));
        } else if (offHandItems.getCount() < 32) {
    ...
    public class Spell implements RenderedSpell {
		...
		public void cast(CastingEnvironment ctx) {
            caster.setStackInHand(
                Hand.OFF_HAND,
                caster.getOffHandStack().getCount() < 32 ?
                    ItemStack.EMPTY :
                    new ItemStack(
            ...
            ((LapisworksInterface)this.entity).setFallDmgRes(
                ((LapisworksInterface)this.entity).checkFallDmgRes() + 1
            );
		}
