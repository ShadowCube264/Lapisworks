...
public class EnchantLightning implements SpellAction {
    public int getArgc() {
        return 1;
    }
    ...
    public SpellAction.Result execute(List<? extends Iota> args, CastingEnvironment ctx) {
        ...
        if (((LapisworksInterface)entity).checkLightningBending() > 2) {
            MishapThrowerJava.throwMishap(
                new MishapAlreadyHasEnchantment(
                    entity,
                    Text.translatable("lapisenchantments.lapisworks.lightningbending"),
                    0,
                    2
                )
            );
        ...
        if (offHandItems.isEmpty()) {
            ...
            MishapThrowerJava.throwMishap(MishapBadOffhandItem.of(offHandItems, "amel"));
        } else if (offHandItems.getCount() < 64) {
            MishapThrowerJava.throwMishap(new MishapNotEnoughOffhandItems(offHandItems, 64));
        }

        return new SpellAction.Result(
            new Spell(entity, caster),
            MediaConstants.CRYSTAL_UNIT * 20,
    ...
    public class Spell implements RenderedSpell {
		...
		public void cast(CastingEnvironment ctx) {
            ...
            ((LapisworksInterface)this.entity).setLightningBending(
                ((LapisworksInterface)this.entity).checkLightningBending() + 1
            );
		}
